...
面向城市绿化养护单位的一体化记录系统：以**绿地台账**为主线，串联**养护任务登记**、**养护记录录入**、**绿植更换记录**与**补植成活跟踪**，并提供养护总览看板。
...
| 补植成活 | `/replanting` | 补植登记（苗木来源、供苗单位与批次、数量、规格、约定复核日期）、到期复核登记成活数量与死亡原因、自动核算成活率、按供苗单位/批次汇总并标记偏低、待复核与逾期提醒 |
...
│   │   │   ├── replanting_records.py
│   │   │   ├── statistics.py
│   │   │   └── meta.py
│   │   ├── schemas/             # 校验层：写库字段校验 + 查询条件解析
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / replanting_record.py
...
│   │   └── views/               # 视图：dashboard / green-space / task / record / replacement / replanting
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、19 条任务、26 条养护记录、8 条更换记录、9 条补植记录，含待复核/逾期/成活率偏低供苗单位场景）。停止与清理：
...
| GET/POST | `/replanting-records` | 补植列表（`green_space_id`/`plant_category`/`review_status`/`supplier`/`batch_no`/`low_survival`/日期区间/关键字，返回汇总） / 登记补植 |
| GET/PUT/DELETE | `/replanting-records/{id}` | 详情 / 更新 / 删除 |
| PATCH | `/replanting-records/{id}/review` | 约定复核期后登记成活数量与死亡原因，自动核算成活率 |
| GET | `/replanting-records/summary` | 补植汇总（按供苗单位、供苗批次；成活率偏低时打 `low_survival` 标记） |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，补植记录 `RP-YYYYMMDD-序号`；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
4. **日期约束**：养护日期、更换日期、补植日期不得早于绿地建成日期；约定复核日期不得早于补植日期。
5. **金额核算**：更换金额 = 数量 × 单价，由后端统一计算；未填单价时金额留空，前端提示补录。
6. **补植成活跟踪**（`replanting_service`）：
   - 登记补植时记录苗木来源（供苗单位、供苗批次）、数量、规格与**约定复核日期**，此时状态为「待复核」；超过约定复核日期仍未复核的自动呈现「逾期未核」（由复核日期动态判定，不入库）；
   - 复核时成活数量与复核日期必须同时登记，成活数量不能大于补植数量、复核日期不能早于补植日期或晚于今天；存在死亡苗木时必须登记主要死亡原因；
   - 成活率 = 成活数量 ÷ 补植数量 ×100%，死亡数量 = 补植数量 − 成活数量，均由后端统一计算；
   - 按**供苗单位**与**供苗批次**分别汇总成活率，已复核批次成活率低于 **85%** 时在汇总结果中打 `low_survival` 标记并红色预警；待复核批次不计入成活率分母；
   - 看板「待复核补植作业」列出已到（含超过）约定复核日期且未复核的记录。
7. **删除保护**：删除绿地时若已存在任务/记录/更换/补植数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务、养护记录时关联的补植记录默认保留（解除关联），避免成活履历丢失。
...
| `replanting_record` | 补植作业记录 | `replanting_no`(唯一)、`green_space_id`、`maintenance_record_id`(可空)、`plant_name`、`spec`、`quantity`、`unit`、`supplier`、`batch_no`、`replant_date`、`review_due_date`、`reviewed_date`、`survivor_quantity`、`death_reason`（成活率/死亡数量/复核状态为派生字段） |

绿地删除时任务/记录/更换/补植级联清理；任务与养护记录之间、养护记录与更换/补植记录之间为可空外键（`SET NULL`），保证养护与成活履历可独立留存。

## 八、测试

```bash
cd backend
python -m pytest              # 77 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、补植编号生成、复核字段联动（成活数量/死亡原因/日期约束）、复核状态（待复核/逾期/已复核）动态判定、成活率核算与偏低标记、按供苗单位与批次汇总口径、删除保护与强制删除、统计聚合口径一致性、演示数据自洽性。
